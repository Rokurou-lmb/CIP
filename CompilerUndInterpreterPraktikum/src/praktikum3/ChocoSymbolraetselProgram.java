...
public class ChocoSymbolraetselProgram {
	public static void main(String args[]) {
		...
		solver.solve();
		// Print name value
		System.out.println("donald = " + solver.getVar(donald).getVal());
		System.out.println("gerald = " + solver.getVar(gerald).getVal());
		System.out.println("robert = " + solver.getVar(robert).getVal());
	}
	
	private static int columnConstraint(Model model, IntegerVariable firstLetter, IntegerVariable secondLetter, IntegerVariable thirdLetter, int carryover) {
		IntegerExpressionVariable sum = Choco.plus(carryover, Choco.plus(firstLetter, secondLetter));
//		model.addConstraint(Choco.mod(v0, v1, c)); modulo?
		return 0;
	}
}
